...
```{r}
reg_dates <- seq.Date(today()-years(10), today(), by = "month")
flat_registrations <- c(100+10*rnorm(length(reg_dates)-24))
increasing_registrations <- 100*1.03^(1:24)+10*rnorm(24)
tbbl <- tibble(reg_dates, `New Registrations`=round(c(flat_registrations, increasing_registrations)))|>
  mutate(`Apprentice Completions`=.5*`New Registrations`,
         comp_dates=reg_dates+months(24))

tbbl <- inner_join(tbbl|>select(reg_dates, `New Registrations`), 
                   tbbl|>select(comp_dates, `Apprentice Completions`), by=c("reg_dates"="comp_dates"))|>
  rename(dates=reg_dates)
...
## No real indication of congestion...
...
plt <- ggplot(mapping=aes(completion_cor, 
  ...
  geom_point(data=congestion|>filter(completion_cor < -.5, delay_cor > .5), colour="red", size = 3)+
